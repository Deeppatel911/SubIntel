...
public class TransactionService {
	...
	public List<TransactionDTO> getTransactionsForCurrentUser() {//ResponseEntity<?> getTransactionsForCurrentUser() {
		try { // Add try block
            ...
            return transactions.stream()
                    .map(entity -> new TransactionDTO(
                        ...
                        entity.getLocalDate(),
                        entity.getCategory(),
                        entity.getAccountModel().getName(),
		...
		//String userEmail = SecurityContextHolder.getContext().getAuthentication().getName();
		//UserModel user = userRepository.findByEmail(userEmail).orElseThrow(() -> new UsernameNotFoundException(userEmail));
		//List<TransactionModel> transactions = transactionRepository.findTransactionsByUserId(user.getUser_id());
		//return ResponseEntity.ok(transactions);
	}
}
